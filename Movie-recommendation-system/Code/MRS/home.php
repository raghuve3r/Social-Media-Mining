...
<a href="logout.php?logout"><span style="float: right; color: white; font-style: italic";>Sign Out</span></a> 
		...
		<input type="submit" value="Log Off"><br>
			...
			<span style="float: left; font-size:20px;color: white;"> Recommended <strong>Youtube </strong>Videos</span>
			<span style="float: right; font-size:20px;color: white;"> <a style="color:white" href="imdb.php"> Goto <strong>IMDB </strong>Recommendation</a></span>
			<br /> <br />
			<?php
...
and user_id <> '".$_SESSION['user']."' and rating = 5 ORDER BY RAND() LIMIT 9");
...
order by IMDB_Rating, Rand()desc LIMIT 5");

echo "<table border=1>";

while ($row = mysql_fetch_array($imdb_list,MYSQL_ASSOC)) {
echo '<a href="http://www.imdb.com/title/'. $row["IMDB_ID"] .'" target="_blank"> <img src= "images/' . $row["IMDB_ID"] . '.jpg" > </a>';
}
